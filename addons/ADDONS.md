...
Module folders use **underscores** (Python / Odoo requirement). There is **no extra nesting** — both addons sit directly under `odoo18_booking_system/`.

`etc/odoo.conf` → `addons_path` includes **one** custom entry: `/mnt/extra-addons/odoo18_booking_system` (Odoo discovers `studio_booking` and `studio_booking_api` as subdirectories).

**Database:** `ir_module_module` still stores `studio_booking` and `studio_booking_api`. Moving or renaming **disk** folders does **not** delete data if `addons_path` stays correct.
